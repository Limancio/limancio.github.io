...
void FPSCamera::move(const glm::vec3& offsetPos, bool noclip)
{
	if (noclip) {
		float old_y = mPosition.y;
		mPosition += offsetPos;
		mPosition.y = old_y;
	}
	else {
...
void OrbitCamera::rotate(float yaw, float pitch)
{
	...
	mPitch = glm::clamp(mPitch, -glm::pi<float>() / 2.0f + 0.01f, glm::pi<float>() / 2.0f - 0.01f);
