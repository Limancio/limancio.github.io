...
void main()
{
    ...
    for(int i = 0; i < nrPointsLights/*NR_POINT_LIGHTS*/; i++)
    ...
    frag_color = vec4(result, 1.0) * GenerateTerrainColor();
	frag_color = mix(vec4(skyColour, 1.0), frag_color, Visibility);
}
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
	...
	if(material.useSpecular == 1)
		...
		specular = light.specular * spec * material.specularVec;
...
vec4 GenerateTerrainColor()
{
    ...
    terrainColor += regionWeight * texture2D(region1ColorMap, TexCoords);
    ...
    regionWeight = (regionRange - abs(height - regionMax)) / regionRange;
    regionWeight = max(0.0, regionWeight);
    terrainColor += regionWeight * texture2D(region2ColorMap, TexCoords);
    ...
    regionWeight = (regionRange - abs(height - regionMax)) / regionRange;
    regionWeight = max(0.0, regionWeight);
    terrainColor += regionWeight * texture2D(region3ColorMap, TexCoords);
    ...
    regionWeight = (regionRange - abs(height - regionMax)) / regionRange;
    regionWeight = max(0.0, regionWeight);
    terrainColor += regionWeight * texture2D(region4ColorMap, TexCoords);
